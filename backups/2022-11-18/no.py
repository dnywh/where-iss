...
# My stuff
def load_image(img, w, h, bg):
    epd.Clear(bg)

    epd.fill(bg)

    # Our desired image
    # Image formatted as byte array via image2cpp sized the same as image_width x image_height (324x240)
    ...
    # Center image
    image_x = int((EPD_WIDTH - image_width) / 2)
    image_y = int((EPD_HEIGHT - image_height) / 2)

    # Match image by setting frame buffer to MONO_HLSB
    fb = framebuf.FrameBuffer(
        buffer, image_width, image_height, framebuf.MONO_HLSB)

    # Position the image
    epd.blit(fb, image_x, image_y)
...
if __name__ == '__main__':
    ...
    load_image(image_no, 324, 166, 0x00)

    # Wait X seconds
    epd.delay_ms(5000)

    load_image(image_yes, 324, 264, 0xff)
